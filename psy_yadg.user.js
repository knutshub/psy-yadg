// ==UserScript==
// @id             psy-yadg
// @name           psytorrents.info - YADG
// @description    This script provides integration with online description generator YADG (http://yadg.cc) Based on userscript by Slack06@github (v. 1.3.5).
...
// @include        http*://*psytorrents.info/torrents.php*
// ==/UserScript==
...
*/
var defaultPsytorrentsFormat = 5;
...
var factory = {
    ...
    locations : new Array(
        {
            name : 'psytorrentsinfo_upload',
            regex : /http(s)?\:\/\/(.*\.)?psytorrents\.info\/upload\.php.*/i
        },
        {
            name : 'psytorrentsinfo_edit',
            regex : /http(s)?\:\/\/(.*\.)?psytorrents\.info\/torrents\.php\?action=editgroup&groupid=.*/i
        },
        {
            name : 'psytorrentsinfo_request',
            regex : /http(s)?\:\/\/(.*\.)?psytorrents\.info\/requests\.php\?action=new/i
        },
        {
            name : 'psytorrentsinfo_request_edit',
            regex : /http(s)?\:\/\/(.*\.)?psytorrents\.info\/requests\.php\?action=edit&id=.*/i
        },
        {
            name : 'psytorrentsinfo_torrent_overview',
            regex : /http(s)?\:\/\/(.*\.)?psytorrents\.info\/torrents\.php\?id=.*/i
        }
    ),
    ...
    initializeSettings : function() {
        ...
        if (settings_ver < current_ver) {
            ...
            var locations = [
                'psytorrentsinfo_upload',
                'psytorrentsinfo_request'
            ];
    ...
    setDefaultFormat : function() {
        ...
        if (default_format !== null && default_format in format_offsets) {
            ...
            switch (this.currentLocation) {
                default:
                    format_select.selectedIndex = format_offsets[defaultPsytorrentsFormat];
                    break;
            }
        }
    },
    ...
    getInputElements : function() {
        ...
        switch (this.currentLocation) {
            ...
            case "psytorrentsinfo_torrent_overview":
                var div = document.createElement('div');
                div.className = "yadg_div";
            ...
            case "psytorrentsinfo_request":
            case "psytorrentsinfo_request_edit":
                var tr = document.createElement('tr');
                tr.className = "yadg_tr";
                tr.innerHTML = '<td class="label">YADG:</td><td>' + inputHTML + scraperSelectHTML + scraperInfoLink + buttonHTML + toggleOptionsLinkHTML + optionsHTML + responseDivHTML + '</td>';
                return tr;
